...
const columns = [
    { headerName: "OwnerName", field: "OwnerName", flex: 1 },
    { headerName: "ContactPrimary", field: "ContactPrimary", flex: 1 },
...
const Component = (props) => {
    ...
    const FetchResults = async () => {
        ...
        await Api.GetPetOwnersCount(query)
            .then(async (res) => {
                if (res.status) {
        ...
        await Api.GetPetOwnersMulti(query, "")
            .then(async (res) => {
                if (res.status) {
                    _rows = res.values || [];
                    if (_rows.length > 0) {
    ...
    const OnCloseClicked = async (e) => {
        if (e) {
            const rslt = await Api.SetPetOwnerSingle({ OwnerId: deletedId, Deleted: true });
            if (rslt.status) {
                setInitialize(true);
    ...
    const OnActionClicked = (id, type) => {
        let _route;
        if (type === 'edit') _route = `/PetOwners/edit/${id}`;
        if (type === 'view') _route = `/PetOwners/view/${id}`;
    ...
    return (
        ...
        <>
            <Container {...props}>
                <Box style={{ width: '100%', paddingBottom: 5 }}>
                    ...
                    <Stack direction="row">
                        <Grid container sx={{ justifyContent: 'flex-end' }}>
                                ...
                                onClick={() => NavigateTo("/PetOwners/create")}
                            >
                                <AddBoxIcon />
                            </IconButton>
                        </Grid>
                ...
                <Box style={{ width: '100%' }}>
                    <DataTable keyId={'OwnerId'} columns={columns} rowsCount={rowsCount} rows={rows} sortBy={sortBy} pageInfo={pageInfo} onActionClicked={OnActionClicked}
